...
Page({
	...
	wechatDecryptData() {
		...
		App.WxService.login()
		.then(data => {
			console.debug('wechatDecryptData', data.code);
			code = data.code;
	...
	wechatSignIn(cb) {
		...
		App.WxService.login()
		.then(data => {
			console.debug('wechatSignIn', data.code);
		...
		.then(res => {
			...
			console.debug('wechatSignIn', data);
	...
	wechatSignUp(cb) {
		App.WxService.login()
		.then(data => {
			console.debug('wechatSignUp', data.code);
		...
		.then(res => {
			const data = res.data;
			console.debug('wechatSignUp', data);
